...
export default function Products() {
    ...
    return (
        <div className='mt-32'>
            ...
            <div className='grid xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mt-20 place-items-center pb-20'>
                {coffes.map((coffee: CoffeeProps, index: number) =>
                    <div key={coffee.id} className='relative rounded-md rounded-tr-[2rem] rounded-bl-[2rem] bg-alt min-h-[310px] max-w-[256px]'>
                        ...
                        <div className='flex flex-col px-5 pb-5'>
                            <div className='flex justify-center gap-1'>
                                {coffee.variation.map((item) => (
                                    <span key={index} className='inline text-center py-1 px-2 text-yellowDark bg-yellowLight rounded-[30px] max-h-5 max-w-20 text-[0.625rem] font-bold uppercase translate-y-[-10px]'>{item.name}</span>
                                ))}
                            </div>
